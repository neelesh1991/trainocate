...
//$password = random_string('alnum', 10);
		...
		if(!empty($result))
		{
			...
			/*	$createdDateVal = new DateTime($row["created"], new DateTimeZone('GMT') );
                ...
                <a onclick="edit_user('.$row['id'].');" class="btn menu-icon vd_bd-yellow vd_yellow" data-placement="top" data-toggle="tooltip" data-original-title="edit"> <i class="fa fa-pencil"></i> </a>
                 ...
                 <a href="'.base_url().'users/reset_exam/'.$row['id'].'" class="btn menu-icon vd_bd-yellow vd_yellow" data-placement="top" data-toggle="tooltip" data-original-title="reset exam"> <i class="fa fa-repeat"></i> </a>

                  	</div>';

                  	// onclick="reset_exam('.$row['id'].');"
	...
	public function reset_exam($userid)
	{ 
	    $res = array('user_id'=>$userid);
	    $data['user_id']= $userid;
		$data['exam_Logs']=$this->modelbasic->get_quiz_log_users_using_userid($userid);
		$data['page_name']='user/user_exam_view';
		$this->load->view('index',$data);

	}

	public function user_reset_exam($userid,$examid)
	{ 
	    $preivouwQuizLog=$this->modelbasic->get_previousQuizLog('quiz_log',$examid, $userid);
	    // echo "<pre/>@@@@@"; print_r(end($preivouwQuizLog)); die;

	    $rev_id = $this->modelbasic->_insert('quiz_retry_log',end($preivouwQuizLog));
	    // echo "<pre/>@@@@@"; print_r($rev_id); die;

		if(!empty($preivouwQuizLog)){
			$res = $this->modelbasic->previous_user_quiz_del('quiz_log',array('exam_id'=>$examid,'user_id'=>$userid));
		}
		return TRUE;
	}
		     	...
		     	if(!empty($all_data))
		     	{
		     		...
		     		foreach($all_data as $key => $val)
		     		{
		     			...
		     			if(!empty($val[0]) && !empty($val[1]))
		     			{
		     				...
		     				// $password = random_string('alnum', 10);
		     				$password = "Test123";
		     				$user_info=array('tenant_id'=>$this->session->userdata('tenant_id'),'name'=>$val[0],'email_id'=>$val[1],'address'=>$val[2],'age'=>$val[3],'institute_name'=>$val[4],'created'=>date('Y-m-d H:i:s'),'status'=>1,'academic_year'=>$val[6],'principal_name'=>$val[5],'password'=>md5($password),'group_id'=>$groupId,'timezone'=>$timezone,'is_profile_completed'=>1,'reset_password'=>1);
