...

{
	...
	public function getAjaxdataObjects()
	{
		...
		$result=$this->modelbasic->run_query('tenant as A',$requestData,$columns,$selectColumns,'','',$condition,$data_array,$group_by);
		...
		public function add()
		{
			...
			if ($this->form_validation->run())
			{
				...
				if($_FILES['logo']['name']!='')
				{
					...
					$insertDataTenant=array('name'=>$this->input->post('name',TRUE),'header_color'=>$header_color,'created'=>date('Y-m-d H:i:s'),'url'=>$this->input->post('url',TRUE),'timezone'=>$this->input->post('time_zone',TRUE),'signup_permission'=>$this->input->post('signup_permission',TRUE),'address'=>$this->input->post('address',TRUE),'signup_permission'=>$this->input->post('signup_permission',TRUE),'bind_email'=>$this->input->post('bind_email',TRUE),'email_postfix'=>$this->input->post('email_postfix',TRUE),'bind_organization'=>$this->input->post('bind_organization',TRUE));
...
if($res >0)
					...
					$updateRes=$this->modelbasic->_update('tenant',$res1, $updateData);
					if($res1 > 0)
					{
				...
				if($this->input->post('id',TRUE)!='')
				{
					...
					if($_FILES['logo']['name']!='')
					{
						...
						$data=array(
							...
							'bind_email'=>$this->input->post('bind_email',TRUE),
							'email_postfix'=>$this->input->post('email_postfix',TRUE),
							'bind_organization'=>$this->input->post('bind_organization',TRUE),
