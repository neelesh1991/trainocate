	...
	function add_visit()
	{
		...
		else
		{
			...
			if($result->num_rows() == 1)
			{
				...
				$visit_entry_visit_id=$visit['visit_id'];
			}
			else
			{
				$visit_entry_visit_id='';
		...
		$this->db->select('count,visit_id,visit_visit_date');
		$this->db->from($this->visit_table);
		$this->db->where('visit_visitor_id', $visitor_id);
		$this->db->where('visit_is_external_referral', $is_external_referral);
		$this->db->where('visit_is_search_referral', $is_search_referral);
		$this->db->where('visit_uri', uri_string());
		$this->db->where('visit_referrer', $referrer);
		$this->db->where('visit_is_direct_access', $is_direct_access);
		$this->db->like('visit_visit_date',date('Y-m-d'));
		...
		if(count($data) > 0)
		{
			...
			//$this->db->like('visit_visit_date',date('Y-m-d'));
			$this->db->update($this->visit_table);
		}
		else
		{
			...
			$this->db->set('visit_entry_visit_id', $visit_entry_visit_id);
			$this->db->set('count', 1);
